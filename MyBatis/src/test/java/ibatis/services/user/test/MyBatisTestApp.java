...
public class MyBatisTestApp {
	...
	public static void main(String[] args) throws Exception{
		...
		Reader reader = Resources.getResourceAsReader("sql/mybatis-config.xml"); //inputStream과의 차이점
		
		//==> 2. Reader 객체를 이용 xml metadata 에 설정된 각정 정보를 접근, 사용가능한 
		//==>     SqlSession을 생성하는 SqlSessionFactory  instance 생성
		SqlSessionFactory sqlSessionFactory =  new SqlSessionFactoryBuilder().build(reader);
		SqlSession sqlSession = sqlSessionFactory.openSession();//java 인터페이스, 명령 실행 및 매퍼를 가져오고 트랜잭션을 관리할 수 있음
