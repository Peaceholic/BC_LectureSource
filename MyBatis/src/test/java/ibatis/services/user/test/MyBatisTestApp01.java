...
public class MyBatisTestApp01 {
	...
	public static void main(String[] args) throws Exception{
		...
		Reader reader = Resources.getResourceAsReader("sql/mybatis-config01.xml"); //inputStream과의 차이점
		
		//==> 2. Reader 객체를 이용 xml metadata 에 설정된 각정 정보를 접근, 사용가능한 
		//==>     SqlSession을 생성하는 SqlSessionFactory  instance 생성
		SqlSessionFactory sqlSessionFactory =  new SqlSessionFactoryBuilder().build(reader);
		SqlSession sqlSession = sqlSessionFactory.openSession(true);//java 인터페이스, 명령 실행 및 매퍼를 가져오고 트랜잭션을 관리할 수 있음
		
		System.out.println("\n");
		
		System.out.println(":: 0. all User(SELECT) ? ");
		//==> 3. xml 에 등록된 query 수행 후 설정된 출력(?) 객체를 갖는 List 객체 받기 (여러개)
